...
detects the package dependencies as well and provides them in the code
(`libs = TRUE`). 
...
So far, we've been using "prosepctive" provenance generated from the
...
script. To do so, we can pass the provenance to the `clean` function
via `options`:
